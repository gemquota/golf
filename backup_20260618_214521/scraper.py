...
import config, db, terminal, server
...
def worker(chunk_id, stats, stats_lock, total_tasks, ip_score, record_raw, worker_count, tasks):
    ...
    for url, username, password in tasks:
        ...
        while server.PAUSED and not server.STOPPED: time.sleep(0.5)
        ...
        try:
            ...
            terminal.update_display({"index": idx, "successes": stats["successes"], "total_bonuses": stats["total_bonuses"],
            ...
            terminal.update_display({"index": idx, "successes": stats["successes"], "failures": stats["failures"],
...
def run_scrape():
    resume = "-r" in sys.argv; shuffle = "-s" in sys.argv
    record_raw = "-v" in sys.argv and sys.argv[sys.argv.index("-v")+1] == "max" if "-v" in sys.argv else False
    ...
    for aidx, (username, password) in enumerate(accounts):
        if not urls or server.STOPPED: break
        tasks = [(u, username, password) for u in urls]
        nw = min(10 if proxies else 1, len(tasks))
        terminal.print_launcher(len(tasks), len(proxies), nw, shuffle, ip_score)
