...
SPECTRUM = [(0,255,255,0),(100,0,255,0),(200,0,100,0),(250,0,0,139),(300,0,0,255),
    (400,75,0,130),(500,128,0,128),(600,255,140,0),(700,255,165,0),(800,255,0,0),
    (900,255,255,0),(1000,0,128,128),(1100,238,130,238)]
...
def print_launcher(tasks, proxies, workers, shuffle, ip_score):
    ...
    try: _cpu = f"[white]{psutil.cpu_percent()}% / RAM {psutil.virtual_memory().percent}%[/]"
    except: _cpu = "N/A"
    ...
    legend = "\u2705 OK\n\U0001f47b 404\n\U0001f6ab 403\n\U0001f69a 301\n\U0001f4e1 101\n\u2601\ufe0f 503\n\U0001f40c Lag\n\U0001f4c9 Track"
    mid = Table.grid(expand=True); mid.add_column(ratio=1); mid.add_column(width=14); mid.add_column(width=14)
    top = Table.grid(expand=True); top.add_column(ratio=1); top.add_column(ratio=1); top.add_column(ratio=1)
    top.add_row(
...
def update_display(d):
    ...
    if msg.startswith("\u2705"): sfmt, col = "\u2705DONE\u2705", "bright_green"
        ...
        if m:
            c = m.group(1); icon = "\U0001f69a" if c=="301" else "\U0001f4e1" if c=="101" else "\U0001f4bb"
    ...
    line = (f"[{_ec(idx,total)}]{idx:03d}[/][{col}]{sfmt}[/][{col}]{STREAK_CNT[0]:02d}[/] "
        f"[{_sc(r5)}]{_rv(r5)}[/] [{_sc(r10)}]{_rv(r10)}[/] [{_sc(r20)}]{_rv(r20)}[/] [{_sc(r50)}]{_rv(r50)}[/] "
        f"[{_sc(r100)}]{_rv(r100)}[/] [{_sc(rate)}]{int(rate):03d}[/] "
...
def print_completion(stats):
    ...
    lines = ["[bold green]Scraping Complete[/]",
